import { db, SenderType, Prisma } from '@clerk/db';
...
async function processInboundMessage(organizationId, platformId, channelType, messageText, aiSummaryQueue, { customerName = null, profilePicUrl = null, initialTag = null, externalMessageId = null, traceId = null, attachments = [] } = {}) {
...
export async function generateThreadIntelligence(threadId) {
    try {
...
export async function sendWhatsAppPlanNotification(organizationId, threadId, customerName, channelType, aiSummary) {
    try {
        ...
        for (const member of members) {
            try {
                ...
                logger.error({ err: sendErr.message, phoneNumber: member.phoneNumber }, '[Worker] Failed to send WhatsApp');
            }
        }
    }
    catch (err) {
        logger.error({ err: err.message, threadId }, '[Worker] sendWhatsAppPlanNotification error');
    }
}
...
export function resolveBusinessHoursSettings(raw) {
    return {
        businessHoursEnabled: raw.businessHoursEnabled ?? BUSINESS_HOURS_DEFAULTS.businessHoursEnabled,
        businessHoursDays: raw.businessHoursDays ?? BUSINESS_HOURS_DEFAULTS.businessHoursDays,
        businessHoursStart: raw.businessHoursStart ?? BUSINESS_HOURS_DEFAULTS.businessHoursStart,
        businessHoursEnd: raw.businessHoursEnd ?? BUSINESS_HOURS_DEFAULTS.businessHoursEnd,
        businessHoursTimezoneOffset: raw.businessHoursTimezoneOffset ?? BUSINESS_HOURS_DEFAULTS.businessHoursTimezoneOffset,
    };
}
...
export async function sendAutoAck(organizationId, threadId) {
    try {
        ...
        logger.error({ err: err.message, threadId }, '[Worker] sendAutoAck error');
    }
}
export async function handleIgDmJob(job, aiSummaryQueue) {
    ...
    try {
        ...
        try {
            ...
            if (integration?.accessToken) {
                const profileRes = await fetch(`${FB_GRAPH}/${senderId}?fields=name,profile_pic&access_token=${integration.accessToken}`);
                if (profileRes.ok) {
            ...
            logger.warn({ err: profileErr.message, senderId }, '[Worker] Failed to fetch IG profile');
        }
...
export async function handleEmailJob(job, aiSummaryQueue) {
    const { organizationId, traceId } = job.data;
...
export async function handleShopifyJob(job, aiSummaryQueue) {
    const { organizationId, traceId } = job.data;
    const { topic, rawPayload } = job.data;
