...
export async function startWorkerRuntime() {
    ...
    const sharedProducerConn = new IORedis(redisUrlStr);
    ...
    const sharedWorkerConn = new IORedis(redisUrlStr, { maxRetriesPerRequest: null });
    ...
    const messageWorker = new Worker(QUEUE.INBOUND, async (job) => {
    ...
    const aiSummaryWorker = new Worker(QUEUE.AI_SUMMARY, async (job) => {
        ...
        const bizSettings = resolveBusinessHoursSettings((org?.settings ?? {}));
