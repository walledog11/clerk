...
export async function createMaintenanceWorkers(workerConn, producerConn) {
    ...
    await tokenHealthQueue.add(JOB.TOKEN_HEALTH_CHECK, {}, { repeat: { every: ONE_DAY_MS }, jobId: JOB.TOKEN_HEALTH_ID });
    const tokenHealthWorker = new Worker(QUEUE.TOKEN_HEALTH, async () => {
        ...
        for (let i = 0; i < igIntegrations.length; i += CONCURRENCY) {
            await Promise.all(igIntegrations.slice(i, i + CONCURRENCY).map(async (integration) => {
                try {
                    const res = await fetch(`${FB_GRAPH}/${integration.externalAccountId}?fields=id&access_token=${integration.accessToken}`);
                    ...
                    if (integration.refreshToken && appId && appSecret) {
                        try {
                            const refreshRes = await fetch(`${FB_GRAPH}/oauth/access_token?grant_type=fb_exchange_token&client_id=${appId}&client_secret=${appSecret}&fb_exchange_token=${integration.refreshToken}`);
                            ...
                            logger.warn({ organizationId: integration.organizationId, err: refreshErr.message }, '[TokenHealth] User token refresh error — page token still valid');
                        }
                    }
                    await db.integration.update({
                        where: { id: integration.id },
                    ...
                    logger.error({ organizationId: integration.organizationId, err: err.message }, '[TokenHealth] Failed to check token');
                }
            }));
        }
    ...
    await archivalQueue.add(JOB.ARCHIVE_THREADS, {}, { repeat: { every: ONE_DAY_MS }, jobId: JOB.ARCHIVE_THREADS_ID });
    ...
    await purgeQueue.add(JOB.PURGE_DELETED, {}, { repeat: { every: ONE_DAY_MS }, jobId: JOB.PURGE_DELETED_ID });
    ...
    await digestQueue.add(JOB.DIGEST, {}, { repeat: { every: ONE_HOUR_MS }, jobId: JOB.DIGEST_ID });
    ...
    function shouldSendDigest(settings, currentHourUtc) {
        ...
        if (days === 'weekdays' && (localDay === 0 || localDay === 6))
            return false;
        if (frequency === 'daily')
            return localHour === firstHour;
        if (frequency === 'twice_daily')
            return localHour === firstHour || localHour === secondHour;
    ...
    const digestWorker = new Worker(QUEUE.DIGEST, async () => {
        ...
        for (const t of allOpenThreads) {
            if (!threadsByOrg.has(t.organizationId))
                threadsByOrg.set(t.organizationId, []);
        ...
        for (const org of eligibleOrgs) {
            ...
            if (urgent > 0)
                lines.push(`  No reply >24h: ${urgent}`);
            if (stale > 0)
                lines.push(`  Needs attention (4-24h): ${stale}`);
            if (fresh > 0)
                lines.push(`  Recent (<4h): ${fresh}`);
            if (topTags)
                lines.push(``, `Topics: ${topTags}`);
            ...
            for (const member of org.members) {
                try {
                    ...
                    logger.error({ err: e.message, phone: member.phoneNumber }, '[Digest] Failed to send digest');
                }
            }
        }
    }, { connection: workerConn });
