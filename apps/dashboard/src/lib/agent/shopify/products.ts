...
export async function searchShopifyProducts(
  ...
  try {
    ...
    if (products.length === 0) return `No products found matching "${query}".`;

    return JSON.stringify(products.map(serializeProduct));
  } catch (err) {
    return formatShopifyToolError("could not search products", err);
  }
}
