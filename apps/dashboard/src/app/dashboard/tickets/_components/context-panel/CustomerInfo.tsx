...
export function CustomerInfo({ customer, isEditing, onEditingChange, onSaved }: CustomerInfoProps) {
  ...
  const makeDraft = useCallback((): EditState => ({
    ...
    note:       customer.note       ?? '',
  }), [addr?.address1, addr?.city, addr?.country_name, addr?.province, addr?.zip, customer.email, customer.first_name, customer.last_name, customer.note, customer.phone])
  ...
  if (isEditing) {
    ...
    const labelCls = "block text-[10px] text-white/30 mb-0.5"
