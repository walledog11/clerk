...
import { RefreshCw, Pencil, ExternalLink, ShoppingBag, X, Search, UserPlus, Check } from "lucide-react"
    ...
    <div>
      ...
      {isEditing ? (
        <input
          ...
          className="w-full text-xs text-slate-800 bg-white border border-slate-200 rounded px-1.5 py-1 focus:outline-none focus:border-slate-400"
        />
      ) : (
        <p className="text-xs text-slate-700">{(draft[key] as string) || <span className="text-slate-300">—</span>}</p>
      )}
    </div>
  )
...
function ShopifySection({
  ...
  if (isEmailThread && !isLoading && !data?.customer) {
    return <p className="text-xs text-slate-400 italic">No Shopify account found for this email.</p>
  }
  ...
  if ((isEmailThread || isLinked) && data?.customer) {
    return (
      <div className="space-y-4">
        ...
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Orders</p>
    ...
    <aside className="w-92 shrink-0 border-l border-slate-200 flex flex-col overflow-y-auto bg-white">
      ...
      <div className="flex flex-col items-center text-center px-4 pt-6 pb-5 border-b border-slate-100 gap-2.5">
        ...
        <div className="flex items-center gap-1.5 border border-slate-200 rounded-full px-2.5 py-1 bg-slate-50">
          ...
          <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">{channel.name}</span>
        </div>
      </div>
      ...
      <div className="px-4 py-4 border-b border-slate-100">
        ...
        {ticket.aiSummary ? (
          <p className="text-xs text-slate-500 leading-relaxed">{ticket.aiSummary}</p>
        ) : (
          <p className="text-xs text-slate-300 italic leading-relaxed">No summary yet — click refresh to generate one.</p>
        )}
      </div>
      ...
      {hasShopify && (
        <div className="px-4 py-4 border-b border-slate-100">
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <p className="text-xs font-bold text-[#96BF48] uppercase tracking-widest">Shopify</p>
          </div>
          <ShopifySection thread={thread} onLinkShopifyCustomer={onLinkShopifyCustomer} />
        </div>
      )}
      ...
      <div className="px-4 py-4">
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Conversation</p>
        <div className="space-y-2.5">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Topic</p>
            {isEditingTag ? (
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Date Ticket Opened</p>
