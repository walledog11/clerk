        ...
        <div className="divide-y divide-white/[0.06]">
          {inboxFeed.map((item, i) => (
            <div key={i} className="flex items-start gap-3 px-4 py-3.5">
              <div
                className="mt-0.5 shrink-0 size-7 rounded-lg flex items-center justify-center text-white text-[10px] font-extrabold tracking-wide"
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1.5">
                  {item.aiDrafted ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold text-green-300 bg-green-400/10 border border-green-400/30 rounded-full px-2 py-0.5">
                      <Zap className="size-2.5" />
                      AI drafted
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-white/50 bg-white/[0.04] border border-white/10 rounded-full px-2 py-0.5">
                      Pending review
                    </span>
                  )}
                </div>
          ...
          <div className="flex items-center justify-between px-4 py-2.5 bg-white/[0.02]">
            ...
            <span className="text-[11px] font-extrabold text-green-300">
              AI handled 91%
            </span>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 shadow-[0_24px_100px_-56px_rgba(0,0,0,0.95)]">
        <div className="flex gap-0.5 mb-2.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} className="size-3 text-green-400 fill-green-400" />
          ))}
        </div>
        ...
        <div className="mt-3.5 flex items-center gap-2.5">
          <div className="size-7 rounded-full bg-green-400/15 border border-green-400/30 flex items-center justify-center text-green-300 text-[10px] font-extrabold">
