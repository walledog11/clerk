...
- Mock Stripe, Shopify, Clerk, Meta, Twilio, Postmark, Redis, and network `fetch`; no live provider calls in tests.
...
Run `npm run test:e2e:smoke` for changes touching billing, auth, org isolation, messages, agent approval, tickets UI, integrations, or middleware. Browser-auth E2E remains release/nightly or manual unless the changed surface requires real Clerk browser auth.
