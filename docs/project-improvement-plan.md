...
| A3 | `in_progress` | **The critical path.** The controlled account accepted inbound text/image, SocialAPI accepted a reply, its outgoing row appeared in a bounded read, and the participant confirmed receipt; see the dated spike evidence. The send-result ID still does not match either outgoing-row ID. A typed client, guarded spike runner, strict V2 verifier, and content-free gateway observation route are locally verified. No webhook was registered during the exchange, and the route is not deployed or wired into Shopkeeper persistence/approval. Next: deploy/register the receiver, capture fresh signed `dm.received`/`dm.sent` identity evidence, then implement the production workflow and reconnect exercise. Vendor/data-processing diligence remains mandatory before external merchant data. Direct Meta is deferred and does not gate this phase. |
