...
Created: 2026-09-07. Last reconciled: 2026-09-10. Status: in progress; A1 and A2 are verified, A4's implementation is landed, B2 is in progress, and Gate 1 remains open. **A3 is the critical path**, and on 2026-09-10 it advanced to the last hop: a real Instagram DM naming an order produced a mutative plan, that plan reached the merchant's phone as an approval card, the merchant approved it, and the turn executed as `human_approved`. The refund itself was blocked by a Shopify currency guard and the agent escalated rather than claiming success, so the round trip is not yet closed — but every leg except the write is now proven in production. The SocialAPI merchant connect has still never run for a live merchant, and is closed by configuration in production.
...
**Sequencing note, 2026-09-09.** Every remaining Gate 1 item except A3 is bounded, understood work on an existing application path. A3 now has isolated client/signature scaffolding but no application wiring or live-provider evidence, and it gates the pilot alone — a certified candidate with no customer channel cannot reach a merchant. So B3, B4, C2, and C1's remaining items are parked, not cancelled: start them when the A3 spike establishes identity and lifecycle semantics, or when A3 is blocked waiting on the vendor and there is free capacity. Do not let a bounded item run first because it is easier to estimate.
